...
static void PlayerHandleBattleMonInfo(u32 battler);
...
static void (*const sPlayerBufferCommands[CONTROLLER_CMDS_COUNT])(u32 battler) =
{
    ...
    [CONTROLLER_MONINFO]                  = PlayerHandleBattleMonInfo,
...
static void HandleInputChooseAction(u32 battler)
{
    ...
    if (JOY_NEW(A_BUTTON))
    {
        ...
        case 0: // Top left
            ...
            break;
        case 1: // Top right
            BtlController_EmitTwoReturnValues(battler, B_COMM_TO_ENGINE, B_ACTION_MON_INFO, 0);
            PlayerBufferExecCompleted(battler);
            break;
        case 2: // Bottom left
            BtlController_EmitTwoReturnValues(battler, B_COMM_TO_ENGINE, B_ACTION_PRAYER, 0);
            break;
        case 3: // Bottom right
...
void HandleInputChooseMove(u32 battler)
{
    ...
    else if (B_MOVE_REARRANGEMENT_IN_BATTLE < GEN_4 && JOY_NEW(SELECT_BUTTON) && !gBattleStruct->zmove.viewing && !gBattleStruct->descriptionSubmenu && !gBattleStruct->isPrayer)
    {
...
static void Intro_TryShinyAnimShowHealthbox(u32 battler)
{
     ...
     && !gBattleSpritesDataPtr->healthBoxesData[BATTLE_PARTNER(battler)].ballAnimActive)
    {
        if (!gBattleSpritesDataPtr->healthBoxesData[battler].healthboxSlideInStarted)
        {
            ...
            DebugPrintfLevel(MGBA_LOG_ERROR, "WHY IS THE SPRITE NOT CREATED BEFORE NOW???");
...
static void Controller_WaitForInfo(u32 battler)
{
    if (gMain.callback2 == BattleMainCB2 && !gPaletteFade.active)
    {
        PlayerBufferExecCompleted(battler);
...
static void PlayerHandleBattleMonInfo(u32 battler)
{
    ...
    gBattlerControllerFuncs[battler] = Controller_WaitForInfo;
}
