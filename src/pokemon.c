...
void CreateBoxMon(struct BoxPokemon *boxMon, u16 species, u8 level, u8 fixedIV, u8 hasFixedPersonality, u32 fixedPersonality, u8 otIdType, u32 fixedOtId)
{
    ...
    SetBoxMonData(boxMon, MON_DATA_EXP, &gExperienceTables[GROWTH_SLOW][level]);
...
u8 GetLevelFromMonExp(struct Pokemon *mon)
{
    ...
    while (level <= MAX_LEVEL && gExperienceTables[GROWTH_SLOW][level] <= exp)
        level++;
...
u32 GetBoxMonData3(struct BoxPokemon *boxMon, s32 field, u8 *data)
{
    ...
    if (field > MON_DATA_ENCRYPT_SEPARATOR)
    {
        ...
        case MON_DATA_SPDEF_EV:
            ...
            break;
        case MON_DATA_OVERWRITTEN_ABILITY:
            retVal = substruct2->overwrittenAbility;
            break;
        case MON_DATA_CUTE:
...
void SetBoxMonData(struct BoxPokemon *boxMon, s32 field, const void *dataArg)
{
    ...
    if (field > MON_DATA_ENCRYPT_SEPARATOR)
    {
        ...
        case MON_DATA_SPDEF_EV:
            ...
            break;
        case MON_DATA_OVERWRITTEN_ABILITY:
            SET16(substruct2->overwrittenAbility);
            break;
        case MON_DATA_CUTE:
...
u16 GetAbilityBySpecies(u16 species, u8 abilityNum, u16 overwrittenAbility)
{
    ...
    if (overwrittenAbility != ABILITY_NONE)
    {
        gLastUsedAbility = overwrittenAbility;
        return overwrittenAbility;
    }
...
u16 GetMonAbility(struct Pokemon *mon)
{
    ...
    return GetAbilityBySpecies(species, abilityNum, ABILITY_NONE);
}
...
void PokemonToBattleMon(struct Pokemon *src, struct BattlePokemon *dst)
{
    ...
    dst->ability = GetAbilityBySpecies(dst->species, dst->abilityNum, GetMonData(src, MON_DATA_OVERWRITTEN_ABILITY, NULL));
    ...
    for (i = 0; i < ITEM_EFFECT_ARG_START; i++)
    {
        ...
        case 3:
             ...
             && GetMonData(mon, MON_DATA_LEVEL, NULL) != MAX_LEVEL)
            {
                ...
                {
                    dataUnsigned = gExperienceTables[GROWTH_SLOW][GetMonData(mon, MON_DATA_LEVEL, NULL) + 1];
                }
                else if (param - 1 < ARRAY_COUNT(sExpCandyExperienceTable)) // EXP Candies
                {
                    ...
                    if (B_RARE_CANDY_CAP && B_EXP_CAP_TYPE == EXP_CAP_HARD)
                    {
                        ...
                        if (dataUnsigned > gExperienceTables[GROWTH_SLOW][currentLevelCap])
                            dataUnsigned = gExperienceTables[GROWTH_SLOW][currentLevelCap];
                    }
                    else if (dataUnsigned > gExperienceTables[GROWTH_SLOW][MAX_LEVEL])
                    {
                        dataUnsigned = gExperienceTables[GROWTH_SLOW][MAX_LEVEL];
                    }
                }
...
bool32 DoesMonMeetAdditionalConditions(struct Pokemon *mon, const struct EvolutionParam *params, struct Pokemon *tradePartner, u32 partyId, bool32 *canStopEvo, enum EvoState evoState)
{
    ...
    for (i = 0; params != NULL && params[i].condition != CONDITIONS_END; i++)
    {
        ...
        case IF_MIN_BEAUTY:
        {
            u32 beauty = GetMonData(mon, MON_DATA_CUTE, 0);
        ...
        {
            u32 coolness = GetMonData(mon, MON_DATA_CUTE, 0);
...
u8 GetTrainerEncounterMusicId(u16 trainerOpponentId)
{
    ...
    return gTrainers[sanitizedTrainerId].encounterMusic_gender & (F_TRAINER_FEMALE - 1);
}
...
bool8 TryIncrementMonLevel(struct Pokemon *mon)
{
    ...
    if (expPoints > gExperienceTables[GROWTH_SLOW][MAX_LEVEL])
    {
        expPoints = gExperienceTables[GROWTH_SLOW][MAX_LEVEL];
        SetMonData(mon, MON_DATA_EXP, &expPoints);
    }
    if (nextLevel > GetCurrentLevelCap() || expPoints < gExperienceTables[GROWTH_SLOW][nextLevel])
    {
        return FALSE;
    }
    else
...
u32 GetFormChangeTargetSpeciesBoxMon(struct BoxPokemon *boxMon, enum FormChanges method, u32 arg)
{
    ...
    if (formChanges != NULL)
    {
        ...
        ability = GetAbilityBySpecies(species, GetBoxMonData(boxMon, MON_DATA_ABILITY_NUM, NULL), ABILITY_NONE);
